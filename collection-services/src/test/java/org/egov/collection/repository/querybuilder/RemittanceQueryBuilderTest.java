...
class RemittanceQueryBuilderTest {
    ...
    void testGetRemittanceSearchQuery() {
        ...
        assertEquals("SELECT * FROM (SELECT *, DENSE_RANK() OVER (ORDER BY rem_id) offset_ FROM (Select rem.bankaccount as"
                        ...
                        + " remRec_tenantId, remRec.id as remRec_id from {schema}.egcl_remittance rem LEFT OUTER JOIN {schema}.egcl_remittancedetails"
                        + " remDet ON rem.id=remDet.remittance LEFT OUTER JOIN {schema}.egcl_remittanceinstrument remIsm ON rem.id=remIsm"
                        + ".remittance LEFT OUTER JOIN {schema}.egcl_remittancereceipt remRec ON rem.id=remRec.remittance  WHERE  rem.tenantId"
    ...
    void testGetRemittanceSearchQuery8() {
        ...
        assertEquals("Select rem.bankaccount as rem_bankaccount,rem.function as rem_function,rem.fund as rem_fund, rem.id"
                        ...
                        + " {schema}.egcl_remittancedetails remDet ON rem.id=remDet.remittance LEFT OUTER JOIN {schema}.egcl_remittanceinstrument"
                        + " remIsm ON rem.id=remIsm.remittance LEFT OUTER JOIN {schema}.egcl_remittancereceipt remRec ON rem.id=remRec.remittance"
    ...
    void testGetRemittanceSearchQuery12() {
        ...
        stringList.add("Select rem.bankaccount as rem_bankaccount,rem.function as rem_function,rem.fund as rem_fund, rem.id"
                ...
                + ",remRec.tenantId as remRec_tenantId, remRec.id as remRec_id from {schema}.egcl_remittance rem LEFT OUTER JOIN"
                + " {schema}.egcl_remittancedetails remDet ON rem.id=remDet.remittance LEFT OUTER JOIN {schema}.egcl_remittanceinstrument"
                + " remIsm ON rem.id=remIsm.remittance LEFT OUTER JOIN {schema}.egcl_remittancereceipt remRec ON rem.id=remRec"
                + ".remittance ");
    ...
    void testGetRemittanceSearchQuery17() {
        ...
        assertEquals("SELECT * FROM (SELECT *, DENSE_RANK() OVER (ORDER BY rem_id) offset_ FROM (Select rem.bankaccount as"
                        ...
                        + " remRec_tenantId, remRec.id as remRec_id from {schema}.egcl_remittance rem LEFT OUTER JOIN {schema}.egcl_remittancedetails"
                        + " remDet ON rem.id=remDet.remittance LEFT OUTER JOIN {schema}.egcl_remittanceinstrument remIsm ON rem.id=remIsm"
                        + ".remittance LEFT OUTER JOIN {schema}.egcl_remittancereceipt remRec ON rem.id=remRec.remittance  WHERE "
